...
Status: **Built and verified in both environments, 2026-09-11/12, and actively extended since** — all 5 build-order steps below are done, in a genuinely separate repo (`bribe-the-band-owner-console`, sibling folder, its own private GitHub repo). See `04-DECISIONS-AND-OPEN-QUESTIONS.md` item 43 for the original full build/verification narrative (including `migration-referral-assigned-at-v1.sql` and a real Stripe-sandbox key wrinkle), item 46 for the affiliate-link cookie fix, item 47 for the promo-code fee-discount window (`migration-promo-fee-discount-v1.sql`), and item 48 for a round of real bugs found and fixed while actually using the tool (a UI layout bug, a server-crash risk on unexpected errors, an environment-URL bug in the Copy Link button). Originally prompted by Travis wanting a real tool for the things he was doing by hand in the Supabase SQL Editor and Table Editor (creating promo codes, editing a performer's plan/discount/referral fields, computing what's owed to an affiliate) — plus reporting that had been on the wishlist since item 25 in `04-DECISIONS-AND-OPEN-QUESTIONS.md` ("Owner reporting — internal/admin-only analytics view," 2026-08-29, never built). This also folds in most of what `06-OWNERS-GUIDE-NOTES.md` was going to have to explain manually — a real console replaces the need to document "how to hand-edit this in Supabase," in Travis's own words.
...
- **Affiliate commission is a percentage of the platform's own 10% cut** (`platform_fee_amount`), not of the performer's gross tip — confirmed. That percentage is editable per code both at creation and after the fact in the owner console, not fixed once set.
- **Retroactive affiliate assignment is going-forward only — nothing backdates, ever, as a rule.** Confirmed: if a performer has been live for a month before an affiliate code gets attached, that month's already-collected revenue does not count toward the affiliate's commission. If a real situation ever calls for an exception, that's a manual, case-by-case adjustment Travis makes himself (e.g. a one-off note in the ledger) — not a feature this tool builds or a toggle it exposes.

## Still open
- **Cowork automation bridge** — which of the two options sketched above (or something else), once the console itself is working and this becomes concrete rather than hypothetical.
...
- ~~Confirmation on the affiliate commission basis and retroactive-attribution timing~~ — **both resolved 2026-09-11**, see "Decisions" above. Nothing else blocks starting.
