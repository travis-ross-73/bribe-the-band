...
Status: **Built and verified in both environments, 2026-09-11/12** — all 5 build-order steps below are done, in a genuinely separate repo (`bribe-the-band-owner-console`, sibling folder, its own private GitHub repo). See `04-DECISIONS-AND-OPEN-QUESTIONS.md` item 43 for the full build/verification narrative, including `migration-referral-assigned-at-v1.sql` (needed for the affiliate commission report, run on both Supabase projects) and a real Stripe-sandbox key wrinkle found and fixed along the way. Originally prompted by Travis wanting a real tool for the things he was doing by hand in the Supabase SQL Editor and Table Editor (creating promo codes, editing a performer's plan/discount/referral fields, computing what's owed to an affiliate) — plus reporting that had been on the wishlist since item 25 in `04-DECISIONS-AND-OPEN-QUESTIONS.md` ("Owner reporting — internal/admin-only analytics view," 2026-08-29, never built). This also folds in most of what `06-OWNERS-GUIDE-NOTES.md` was going to have to explain manually — a real console replaces the need to document "how to hand-edit this in Supabase," in Travis's own words.
