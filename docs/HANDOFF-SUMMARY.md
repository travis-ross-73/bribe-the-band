...
**Staging vs. production, as of 2026-09-10** — fully in sync. A full parity port, the console UI cleanup port that followed it, and chart PDFs becoming optional when adding a song (see `04-DECISIONS-AND-OPEN-QUESTIONS.md`'s 2026-09-09/10 sessions) closed out every gap this section used to track: server-side cooldown enforcement, the Follower account-growth model (since simplified to one identity mechanism), 12-hour gig auto-end, Last Call's front-end, the Follower-facing "gigs I've followed" record, custom SMTP, self-service signup, the console's info-icon/Settings-tab/CSV-export cleanup, and optional chart PDFs are all live on both environments, no known gaps. One caveat: the optional-charts port to production was verified by code/deployment parity, not an independent hands-on login test there — no credentials exist for a real production account. Full detail across all of these: `01-ARCHITECTURE-AND-DATA-MODEL.md` and `04-DECISIONS-AND-OPEN-QUESTIONS.md`.
