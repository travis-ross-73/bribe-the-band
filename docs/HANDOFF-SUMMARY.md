...
**Staging vs. production, as of 2026-09-09** — a full parity port (see `04-DECISIONS-AND-OPEN-QUESTIONS.md`'s 2026-09-09 session) closed out the entire list this section used to track. Current gap:
- **Console UI cleanup** (info-icon popovers replacing always-visible section text, a new Settings tab, setlist reorder-by-number, Reporting CSV export, a few smaller fixes) — built and confirmed on staging 2026-09-09, **not yet ported to production**. Purely front-end, no schema involved, no urgency. Full detail: `01-ARCHITECTURE-AND-DATA-MODEL.md`'s "Console UI cleanup" section and `04-DECISIONS-AND-OPEN-QUESTIONS.md` item 31.
- Everything else that was previously listed here as staging-only — server-side cooldown enforcement, the Follower account-growth model (since simplified to one identity mechanism), 12-hour gig auto-end, Last Call's front-end, the Follower-facing "gigs I've followed" record — is **now live on both environments**. See `04-DECISIONS-AND-OPEN-QUESTIONS.md`'s 2026-09-09 entries for the full port narrative.
- **Custom SMTP** (Resend) and **self-service signup** are both done and confirmed working on both environments — see the 2026-09-08 session entry in `04-DECISIONS-AND-OPEN-QUESTIONS.md`.
