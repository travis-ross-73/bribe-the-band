...
**Staging vs. production, as of 2026-09-10** — fully in sync on code/deployment. A full parity port, the console UI cleanup port that followed it, chart PDFs becoming optional when adding a song, and forgot-password/change-password (see `04-DECISIONS-AND-OPEN-QUESTIONS.md`'s 2026-09-09/10 sessions) closed out every gap this section used to track: server-side cooldown enforcement, the Follower account-growth model (since simplified to one identity mechanism), 12-hour gig auto-end, Last Call's front-end, the Follower-facing "gigs I've followed" record, custom SMTP, self-service signup, the console's info-icon/Settings-tab/CSV-export cleanup, optional chart PDFs, and password recovery/change are all live on both environments. Two caveats, neither a code gap: the optional-charts and password-reset ports to production were verified by code/deployment parity, not an independent hands-on login test there — no credentials exist for a real production account; and the emailed password-reset link won't actually work on **either** environment until Travis adds this console.html URL to each Supabase project's Auth → URL Configuration → Redirect URLs allow-list (dashboard-only, not yet done). Full detail across all of these: `01-ARCHITECTURE-AND-DATA-MODEL.md` and `04-DECISIONS-AND-OPEN-QUESTIONS.md`.
