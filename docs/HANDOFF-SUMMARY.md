...
- ~~Server-side cap + cooldown enforcement~~ — **done, both environments.** Cap was already fine (`enforce_request_cap()` predates this); cooldown had no server-side check at all until now — `enforce_request_limits()`. Confirmed on staging 2026-09-07, run and verified on production 2026-09-08 (a safe, no-regression check against production's own `travis-ross-test` account — see `01-ARCHITECTURE-AND-DATA-MODEL.md`).
