...
**Staging vs. production, as of 2026-09-09/10** — fully in sync. A full parity port plus the console UI cleanup port that followed it (see `04-DECISIONS-AND-OPEN-QUESTIONS.md`'s 2026-09-09 sessions) closed out every gap this section used to track: server-side cooldown enforcement, the Follower account-growth model (since simplified to one identity mechanism), 12-hour gig auto-end, Last Call's front-end, the Follower-facing "gigs I've followed" record, custom SMTP, self-service signup, and the console's info-icon/Settings-tab/CSV-export cleanup are all live on both environments, no known gaps. Full detail across all of these: `01-ARCHITECTURE-AND-DATA-MODEL.md` and `04-DECISIONS-AND-OPEN-QUESTIONS.md`.
