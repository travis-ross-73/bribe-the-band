...
Status: **Fully done, end to end, as of 2026-09-11.** All four build-order steps are complete, and the one real-world prerequisite beyond code/schema — Travis's live-mode Stripe account enrolling as a Connect platform — is also done (item 37). Scoped 2026-09-10, prompted by the new marketing site needing honest copy about how a new performer actually gets paid — today's answer used to be "they reach out and Travis sets it up by hand"; a second real performer can now go through "Connect with Stripe" in Settings and actually complete onboarding for real. See `04-DECISIONS-AND-OPEN-QUESTIONS.md` item 23/36/37 for the monetization numbers this scope builds toward (10% platform fee taken off the top, perpetual affiliate commission by default — both "working numbers, not a permanent commitment," Travis's own words) and the full build/test/port/enrollment narrative.
...
5. ~~Enroll Travis's real live-mode Stripe account as a Connect platform~~ — not originally listed as a build-order step, but turned out to be a real prerequisite beyond code/schema. **Done, 2026-09-11** — see `04-DECISIONS-AND-OPEN-QUESTIONS.md` item 37 for the full path (identity verification, platform-profile acknowledgements, a Stripe support ticket, and a real live-mode test confirming no v2 API migration was needed after all).
